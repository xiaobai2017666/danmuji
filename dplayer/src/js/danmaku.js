...
class Danmaku {
    ...
    load () {
        ...
        if(this.options.api.is===0) {
            url='https://www.bilibili.com/widget/getPageList?aid='+this.options.api.av;
        }else if(this.options.api.is===1) {
            url='https://www.bilibili.com/bangumi/play/'+this.options.api.ep;
        }
        ...
        axios({
            ...
            let cid;
            let result;
            if(that.options.api.is===0) {
                cid=res.data[0].cid;
            }else if(that.options.api.is===1) {
                const reg1=new RegExp(`"aid":${that.options.api.av},"cid":[\\d]+`);
                const reg2=/[\d]+/g;
                result=res.data.match(reg1)[0];
                result=result.match(reg2);
                cid =result[1];
            }

            return axios({
                method: 'GET',
                url: 'https://api.bilibili.com/x/v1/dm/list.so?oid='+ cid +'&type=1',
                responseType: 'document',
                timeout: 3000
            }).then(function(res) {
                let xml=res.data;
                that.dan=Array.prototype.map.call(xml.querySelectorAll('d'),function(item) {
                ...
                that.events && that.events.trigger('danmaku_load_end');
            }).catch((e) => {
                console.error(e);
                that.options.error(e || that.options.tran('Danmaku load failed'));
            });
        });
    }

    reload(av,is,ep) {
        this.options.api.av=av;
        this.options.api.is=is;
        this.options.api.ep=ep;
        this.dan = [];
        this.clear();
        this.load();
    }
